function result = run_step9_confidence_driven_risk_constrained_v6(scenario, params)
%RUN_STEP9_CONFIDENCE_DRIVEN_RISK_CONSTRAINED_V6 Emergency-floor risk scheduler.
%
% Step38 side-copy. It keeps the Step36-selected actual-cost weight and
% enforces a minimum emergency action floor to avoid under-protecting rare
% emergency packets.
...
for k = 1:N
    ...
    [action, meta] = decide_schedule_action_risk_constrained_v6( ...
        scenario.urgency(k), ...
        scenario.q_link(k), ...
        scenario.q_pos(k), ...
...
result.name = "Risk-constrained-v6";
result.policy = policy;
result.delivery = delivery;
end

function [action, chosen_meta] = decide_schedule_action_risk_constrained_v6( ...
...
is_emergency = urgency >= emg_thresh;
...
for i = 1:numel(actions)
    ...
    if is_emergency && is_emergency_floor_action_local(actions(i), params) && obj < best_emg_obj
        best_emg_idx = i;
        best_emg_obj = obj;
        best_emg_aux = aux;
        best_emg_meta = meta;
...
if is_emergency && isfinite(best_emg_obj)
    chosen_idx = best_emg_idx;
    chosen_obj = best_emg_obj;
    chosen_aux = best_emg_aux;
    chosen_meta = best_emg_meta;
    selection_reason = "emergency_floor";
...
function pass = is_emergency_floor_action_local(action, params)
min_priority = paramsafe_local(params, 'risk_v6_emergency_min_priority', 3);
min_rate_mult = paramsafe_local(params, 'risk_v6_emergency_min_rate_multiplier', 2.1);
