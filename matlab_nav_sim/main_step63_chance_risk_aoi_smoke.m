...
% Step63: Chance-risk-AoI side-copy validation.
% This is a candidate-method smoke test. It does not replace
% Risk-constrained-v6 or overwrite Step59-Step62 result files.

base_dir = fileparts(mfilename('fullpath'));
cd(base_dir);

num_seeds = get_env_int_local('STEP63_NUM_SEEDS', 10);
prefix = get_env_string_local('STEP63_PREFIX_TAG', ...
    "step63_chance_risk_aoi_smoke_" + string(num_seeds) + "seed");
cfg = "ModerateForestGeometry";
method_names = ["Link-delay-aware", "Risk-constrained-v6", "Chance-risk-AoI", ...
    "DCC-CBR-window", "AoI-greedy"];

all_scene_matrix = [get_paper_scene_matrix(), get_heldout_scene_matrix()];
scene_filter_raw = getenv('STEP63_SCENES');
...
for scene_idx = 1:numel(run_scene_matrix)
    ...
    fprintf('Step63: scene=%s config=%s seeds=%d\n', scene_meta.scene_id, cfg, num_seeds);

    for seed = seed_list
        [T_seed, T_stage_seed, T_diag_seed] = simulate_step63_seed_local( ...
...
decision = build_step63_decision_table_local(summary, T_diag);
sig_v6 = compute_step63_significance_local(T_raw, run_scene_matrix, cfg, ...
    "Risk-constrained-v6", "Chance-risk-AoI");
sig_link = compute_step63_significance_local(T_raw, run_scene_matrix, cfg, ...
    "Link-delay-aware", "Chance-risk-AoI");
sig_aoi = compute_step63_significance_local(T_raw, run_scene_matrix, cfg, ...
    "AoI-greedy", "Chance-risk-AoI");
...
writetable(sig_v6, prefix + "_v6_vs_chance_risk_aoi_significance.csv");
writetable(sig_link, prefix + "_link_vs_chance_risk_aoi_significance.csv");
writetable(sig_aoi, prefix + "_aoi_greedy_vs_chance_risk_aoi_significance.csv");
write_step63_report_local(decision, sig_v6, prefix + "_report_cn.md", num_seeds);

disp(decision);
fprintf('Step63 complete: Chance-risk-AoI smoke validation generated with %d seeds.\n', num_seeds);

function [T_seed, T_stage_seed, T_diag_seed] = simulate_step63_seed_local( ...
    scene_meta, scene_cfg, cfg, seed, method_names)
...
results = { ...
    ...
    run_step9_chance_risk_aoi_scheduler(scenario, params), ...
...
T_diag_seed = build_step63_diagnostics_row_local(scene_meta, cfg, seed, results{3});
end

function T = build_step63_diagnostics_row_local(scene_meta, cfg, seed, result)
policy = result.policy;
viol = policy.constraint_violation_trace;
feas = policy.feasible_action_ratio_trace;
...
T = table( ...
    ...
    mean(feas, 'omitnan'), mean(feas <= 0.0), ...
    ...
    'MeanFeasibleActionRatio','NoFeasibleActionRate','MeanAoIAge'});
end
...
function decision = build_step63_decision_table_local(summary, T_diag)
...
for i = 1:height(keys)
    ...
    chance = get_method_row_local(subT, "Chance-risk-AoI");
    ...
    row = keys(i, :);
    row.ChanceTimely_pct = 100.0 * chance.TimelyRateMean;
    row.ChanceEmgTimely_pct = 100.0 * chance.EmergencyTimelyRateMean;
    row.ChanceCost = chance.AvgTxCostMean;
    row.TimelyGainVsV6_pp = safe_gain_local(chance, v6, 'TimelyRateMean');
    row.EmgGainVsV6_pp = safe_gain_local(chance, v6, 'EmergencyTimelyRateMean');
    row.CostDeltaVsV6 = safe_raw_delta_local(chance, v6, 'AvgTxCostMean');
    row.TimelyGainVsLink_pp = safe_gain_local(chance, link, 'TimelyRateMean');
    row.TimelyGainVsAoI_pp = safe_gain_local(chance, aoi, 'TimelyRateMean');
    row.ChanceViolationRate_pct = 100.0 * mean(dsub.ChanceViolationRate, 'omitnan');
    row.NoFeasibleActionRate_pct = 100.0 * mean(dsub.NoFeasibleActionRate, 'omitnan');
    row.Decision = classify_step63_case_local(row);
    rows = append_table_local(rows, row);
end
decision = rows;
end

function label = classify_step63_case_local(row)
...
function T_sig = compute_step63_significance_local(T_raw, scene_matrix, cfg, baseline_name, proposed_name)
T_sig = [];
...
function write_step63_report_local(decision, sig_v6, out_path, num_seeds)
fid = fopen(out_path, 'w');
cleanup = onCleanup(@() fclose(fid));
fprintf(fid, '# Step63 Chance-risk-AoI 副本检验报告\n\n');
fprintf(fid, '本步骤为候选方法副本检验，不替换 Risk-constrained-v6 主线。\n\n');
fprintf(fid, '## 运行规模\n\n');
fprintf(fid, '- Seeds: %d\n', num_seeds);
fprintf(fid, '- Config: ModerateForestGeometry\n');
fprintf(fid, '- Proposed candidate: Chance-risk-AoI\n\n');
fprintf(fid, '## 判定结果\n\n');
...
fprintf(fid, '\n## 与 V6 的统计检验\n\n');
fprintf(fid, '已输出 `%s_v6_vs_chance_risk_aoi_significance.csv`，并应用 Holm 校正。\n\n', ...
    erase(string(out_path), "_report_cn.md"));
fprintf(fid, '## 使用边界\n\n');
fprintf(fid, '如果多数场景为 PromisingCandidate 或 NeutralCandidate，才进入 50-seed 扩展验证；如果多数为 UnderperformsV6，则保留 V6 主线。\n');
if nargin >= 2 && ~isempty(sig_v6)
    fprintf(fid, '\n统计检验行数: %d\n', height(sig_v6));
end
end
