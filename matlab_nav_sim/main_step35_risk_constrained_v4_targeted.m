...
% Step35 targeted validation for actual-cost aligned risk v4.
...
prefix = "step35_risk_constrained_v4_targeted_" + string(num_seeds) + "seed";
...
method_names = ["Confidence-heuristic", "Risk-constrained", ...
    "Risk-constrained-v2", "Risk-constrained-v3", "Risk-constrained-v4"];
...
for case_idx = 1:numel(target_cases)
    ...
    for seed = seed_list
        fprintf('Step35 v4 targeted: scene=%s config=%s seed=%d/%d\n', ...
            target.scene_id, target.config, seed, num_seeds);
        [T_seed, T_stage_seed, T_policy_seed] = simulate_step35_case_seed( ...
...
T_policy_summary = build_step35_policy_summary_table(policy_rows);
T_decision = build_step35_decision_table(T_summary);
T_score = build_step35_score_table(T_decision);
...
write_step35_report(T_decision, T_score, T_policy_summary, prefix + "_report.md");

disp(T_decision);
disp(T_score);
fprintf('Step 35 complete: risk-constrained v4 targeted validation generated with %d seeds.\n', num_seeds);

function num_seeds = get_num_seeds_from_env(default_value)
env_value = str2double(getenv('STEP35_NUM_SEEDS'));
...
error('Unknown Step35 scene id: %s', scene_id);
end

function [T_seed, T_stage_seed, T_policy_seed] = simulate_step35_case_seed( ...
...
results = { ...
    ...
    run_step9_confidence_driven_risk_constrained_v4(scenario, params)};

T_seed = evaluate_step9_metrics(scenario, results);
T_seed = add_step35_meta_columns(T_seed, scene_meta, cfg_name, seed);
...
T_stage_seed = add_step35_meta_columns(T_stage_seed, scene_meta, cfg_name, seed);

T_policy_seed = build_step35_policy_table(scenario, results);
T_policy_seed = add_step35_meta_columns(T_policy_seed, scene_meta, cfg_name, seed);
end

function T = add_step35_meta_columns(T, scene_meta, cfg_name, seed)
...
function T_policy = build_step35_policy_table(scenario, results)
rows = [];
...
function T_summary = build_step35_policy_summary_table(T_raw)
...
function T_decision = build_step35_decision_table(T_summary)
keys = unique(T_summary(:, {'Scene','SceneRole','GeometryType','DensityClass','VegetationClass','Config'}), ...
    'rows', 'stable');
methods = ["Risk-constrained", "Risk-constrained-v2", "Risk-constrained-v3", "Risk-constrained-v4"];
rows = [];
...
function T_score = build_step35_score_table(T_decision)
...
function write_step35_report(T_decision, T_score, T_policy_summary, out_path)
fid = fopen(out_path, 'w');
if fid == -1
    error('Could not open Step35 report file: %s', out_path);
end
cleanup_obj = onCleanup(@() fclose(fid)); %#ok<NASGU>

fprintf(fid, '# Step 35 Risk-Constrained V4 Targeted Validation Report\n\n');
fprintf(fid, 'Step35 tests an actual-cost aligned risk objective. ');
fprintf(fid, 'The communication cost used inside the objective is changed from relative rate multiplier cost to the same `mode_cost * tx_rate` metric used by physical delivery evaluation.\n\n');
