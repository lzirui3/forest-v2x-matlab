...
% Step 30 validates the Lyapunov scheduler on the current VTC scene matrix.
% It is a side-copy experiment: no mainline scheduler files are modified.
...
for scene_idx = 1:numel(scene_matrix)
    ...
    for cfg_idx = 1:numel(config_names)
        ...
        fprintf('Step30 Lyapunov validation: scene=%s config=%s seeds=%d\n', ...
...
T_sig_h_vs_l = compute_step30_significance(rows, scene_matrix, config_names, ...
    "Confidence-heuristic", "Confidence-lyapunov");
T_sig_pos_h_vs_l = compute_step30_posdeg_significance(stage_rows, scene_matrix, config_names, ...
    "Confidence-heuristic", "Confidence-lyapunov");
T_sig_link_vs_l = compute_step30_significance(rows, scene_matrix, config_names, ...
...
disp(T_summary);
disp(T_decision);
fprintf('Step 30 complete: Lyapunov VTC validation generated with %d seeds.\n', num_seeds);
