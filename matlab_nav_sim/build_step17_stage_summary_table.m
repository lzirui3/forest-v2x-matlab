function T_summary = build_step17_stage_summary_table(T_raw)
%BUILD_STEP17_STAGE_SUMMARY_TABLE Aggregate scene + config + stage statistics.
    ...
    for c = 1:numel(configs)
        for m = 1:numel(methods)
            for g = 1:numel(stages)
                subT = T_raw(T_raw.Scene == scenes.Scene(s) & T_raw.Config == configs(c) ...
                    & T_raw.Method == methods(m) & T_raw.Stage == stages(g), :);
                row = table( ...
                    scenes.Scene(s), scenes.SceneRole(s), scenes.GeometryType(s), ...
                    scenes.DensityClass(s), scenes.VegetationClass(s), ...
                    configs(c), methods(m), stages(g), ...
                    ...
                    'VariableNames', { ...
                    'Scene','SceneRole','GeometryType','DensityClass','VegetationClass','Config','Method','Stage', ...
