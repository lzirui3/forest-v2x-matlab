...
| **Wave 2 后（+ 完好性 protection，最终）** | **0.99168** | **0.00832** | **2.042** | **1.000** |

净变化：Timely **+0.67pp**、Loss **−44%**、Cost **+10.1%**、Emergency 不变。
**用更有理论依据的方法 + 更接地的投递/约束，换来更好的可靠性与温和的代价上升。**
...
| 编号 | 改动 | 文件 | 结果影响 |
|---|---|---|---|
| S6 | AoI 基线年龄惩罚从线性 `age/cap` 改为**非线性 g(Δ)=norm_age^exponent**（Sun et al. T-IT 2017 凸惩罚，exponent=2，可扫；exponent=1 退回线性） | `compute_action_chance_risk_aoi_objective.m` + `get_default` | 仅影响 AoI 基线；生产 DPP 不变。⚠️ 该基线当前未 thread `context.aoi_age`（age≡0），故形式已接地但激活取决于上游年龄跟踪（既有条件） |
...
2. 若需 AoI 基线真正生效：在 `run_step9_chance_risk_aoi_scheduler` 中 thread `context.aoi_age`（S6 形式已就绪）。
