...
### 多种子验证（②，10 种子，mainline 物理 per_lookup，95% CI）

> 单次 mainline 仿真 ~28 s；下表为 **10 种子**（seed 4200–4209，common random numbers）结果，
> 由 `audit_multiseed_validation.m` 经 `matlab -batch` 跑出（`audit_multiseed_validation.csv` /
> `audit_multiseed_10.log`）。论文级 10–50 种子亦可用既有 per_lookup 重型 harness
> （main_step50/54，含 resume/parallel）。

| 方法 | TimelyRate | LossRate | AvgTxCost | EmgTimely |
|---|---|---|---|---|
| **Proposed-DPP（本文）** | **0.9907 ± 0.0026** | **0.0093 ± 0.0026** | **1.899 ± 0.007** | **1.0000 ± 0.0000** |
| Link-delayaware | 0.9384 ± 0.0054 | 0.0616 ± 0.0054 | 1.630 ± 0.000 | 0.9833 ± 0.0327 |
...
**结论（10 种子，CI 紧，结论稳）**：
1. **唯一 Emergency Timely = 1.0000（±0）的方法**——Link/Heuristic/Oracle 都偶有漏投（0.9833±0.033）。
   这是 DPP 紧急动作下限（priority≥3 + rate≥2.1）的干净差异化。
2. **近 Oracle 可靠性（0.9907 vs 0.9952）而代价低约 10%**（1.899 vs 2.120）。
3. 与启发式同等可靠性（0.9907 vs 0.9902）但**代价低约 10%**（1.899 vs 2.119）。
4. 比 Link-aware **高 5.2pp 可靠性**（0.9907 vs 0.9384）、Loss 仅 1/7。
