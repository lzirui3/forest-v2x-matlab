...
out_prefix = 'step19_quick_per_lookup_validation';
...
for c = 1:numel(configs)
    for p = 1:numel(pdr_modes)
        for s = seed_list
            fprintf('Quick Step19: Config=%s, PdrMode=%s, Seed=%d\n', configs(c), pdr_modes(p), s);
...
writetable(rows, [out_prefix '_raw.csv']);
writetable(stage_rows, [out_prefix '_stage_raw.csv']);
writetable(T_overall, [out_prefix '_overall_delta.csv']);
writetable(T_posdeg, [out_prefix '_posdeg_delta.csv']);
writetable(T_sig, [out_prefix '_overall_significance.csv']);
writetable(T_posdeg_sig, [out_prefix '_posdeg_significance.csv']);

write_step19_per_lookup_validation_report( ...
    T_overall, T_posdeg, T_sig, T_posdeg_sig, [out_prefix '_report.md']);

disp(T_overall);
disp(T_posdeg);
disp(T_sig);
disp(T_posdeg_sig);
disp('Quick Step 19 complete: PER lookup vs sigmoid validation generated.');
