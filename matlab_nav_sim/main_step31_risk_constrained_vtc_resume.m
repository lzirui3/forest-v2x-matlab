...
% Resumable Step31 validation for the risk-constrained side-copy scheduler.
% It writes each scene/config after every seed to avoid losing long runs.
...
for scene_idx = 1:numel(scene_matrix)
    ...
    for cfg_idx = 1:numel(config_names)
        ...
        fprintf('Step31 resume: scene=%s config=%s existing=%d missing=%d\n', ...
...
[T_raw, T_stage_raw] = merge_step31_case_tables(prefix, scene_matrix, config_names);
T_summary = build_step17_summary_table(T_raw);
T_stage_summary = build_step17_stage_summary_table(T_stage_raw);
T_sig_h_vs_r = compute_step31_significance(T_raw, scene_matrix, config_names, ...
    "Confidence-heuristic", "Risk-constrained");
T_sig_link_vs_r = compute_step31_significance(T_raw, scene_matrix, config_names, ...
    "Link-delay-aware", "Risk-constrained");
T_sig_lyap_vs_r = compute_step31_significance(T_raw, scene_matrix, config_names, ...
...
disp(T_summary);
disp(T_decision);
fprintf('Step 31 resumable validation complete: %d seeds x %d scenes x %d configs.\n', ...
...
if isfile(raw_path)
    T_raw = normalize_step31_case_table(readtable(raw_path));
else
    T_raw = [];
end

if isfile(stage_path)
    T_stage = normalize_step31_case_table(readtable(stage_path));
else
    T_stage = [];
end
end

function T = normalize_step31_case_table(T)
if isempty(T)
    return;
end
...
function [T_raw, T_stage_raw] = merge_step31_case_tables(prefix, scene_matrix, config_names)
T_raw = [];
T_stage_raw = [];
...
for scene_idx = 1:numel(scene_matrix)
    ...
    for cfg_idx = 1:numel(config_names)
        ...
        if ~isfile(raw_path) || ~isfile(stage_path)
            error('Missing Step31 case output: %s / %s', raw_path, stage_path);
        end
        T_raw = append_table(T_raw, normalize_step31_case_table(readtable(raw_path)));
        T_stage_raw = append_table(T_stage_raw, normalize_step31_case_table(readtable(stage_path)));
    end
end
end
...
for i = 1:height(keys)
    ...
    elseif timely_gap_pp >= -0.50 && emg_gap_pp >= -0.50
        ...
        interpretation = "Risk-constrained is close enough to heuristic to justify a follow-up.";
