...
% Step50 case worker: fill one held-out scene/config case file only.
% It does not write aggregate Step50 summary/report files, so it can run in
% parallel with the main resumable Step50 script without overwriting totals.
...
if strlength(scene_id) == 0 || strlength(cfg) == 0
    error('STEP50_WORKER_SCENE and STEP50_WORKER_CONFIG must be set.');
end
...
fprintf('Step50 worker: scene=%s config=%s existing=%d missing=%d\n', ...
...
fprintf('Step50 worker complete: scene=%s config=%s seeds=%d.\n', ...
    scene_meta.scene_id, cfg, numel(seed_list));

function [T_seed, T_stage_seed] = simulate_case_seed_local(scene_meta, cfg, seed, method_names, base_params)
params = base_params;
...
elseif cfg == "ForestGeometry"
    ...
    error('Unknown Step50 worker config: %s', cfg);
end
end
