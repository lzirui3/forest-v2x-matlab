function result = run_step9_confidence_driven_risk_constrained_v4(scenario, params)
%RUN_STEP9_CONFIDENCE_DRIVEN_RISK_CONSTRAINED_V4 Actual-cost aligned risk scheduler.
%
% Step35 side-copy. The finite-action selector remains the Step31
% Lagrangian selector, but the objective cost term is aligned with the final
% physical-delivery AvgTxCost metric.
...
for k = 1:N
    ...
    [action, meta] = decide_schedule_action_risk_constrained_v4( ...
        scenario.urgency(k), ...
        scenario.q_link(k), ...
        scenario.q_pos(k), ...
...
result.name = "Risk-constrained-v4";
result.policy = policy;
result.delivery = delivery;
end

function [action, chosen_meta] = decide_schedule_action_risk_constrained_v4( ...
