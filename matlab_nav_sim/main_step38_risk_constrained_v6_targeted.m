...
% Step38 targeted validation for emergency-floor v6.
...
prefix = "step38_risk_constrained_v6_targeted_" + string(num_seeds) + "seed";
seed_list = 1:num_seeds;
scene_matrix = get_paper_scene_matrix();
target_cases = build_target_cases();
method_names = ["Confidence-heuristic", "V4Cost0060", "Risk-constrained-v6"];
...
for case_idx = 1:numel(target_cases)
    ...
    for seed = seed_list
        fprintf('Step38 v6 targeted: scene=%s config=%s seed=%d/%d\n', ...
            target.scene_id, target.config, seed, num_seeds);
        [T_seed, T_stage_seed, T_policy_seed] = simulate_step38_case_seed( ...
...
T_policy_summary = build_step38_policy_summary_table(policy_rows);
T_decision = build_step38_decision_table(T_summary);
T_score = build_step38_score_table(T_decision);
...
write_step38_report(T_decision, T_score, T_policy_summary, prefix + "_report.md");

disp(T_decision);
disp(T_score);
fprintf('Step 38 complete: v6 targeted validation generated with %d seeds.\n', num_seeds);

function num_seeds = get_num_seeds_from_env(default_value)
env_value = str2double(getenv('STEP38_NUM_SEEDS'));
...
error('Unknown Step38 scene id: %s', scene_id);
end

function [T_seed, T_stage_seed, T_policy_seed] = simulate_step38_case_seed( ...
...
T_seed = add_step38_meta_columns(T_seed, scene_meta, cfg_name, seed);
...
T_stage_seed = add_step38_meta_columns(T_stage_seed, scene_meta, cfg_name, seed);

T_policy_seed = build_step38_policy_table(scenario, results);
T_policy_seed = add_step38_meta_columns(T_policy_seed, scene_meta, cfg_name, seed);
end
...
function T = add_step38_meta_columns(T, scene_meta, cfg_name, seed)
...
function T_policy = build_step38_policy_table(scenario, results)
rows = [];
...
for i = 1:numel(results)
    ...
    emergency_floor_share = NaN;
    if isfield(policy, 'selection_reason')
        emergency_floor_share = mean(policy.selection_reason == "emergency_floor");
    end
    row = table(string(result.name), mean(policy.mode == 0), mean(policy.mode == 1), ...
        ...
        mean(policy.priority(idx_emg)), mean(policy.tx_rate(idx_emg)), emergency_floor_share, ...
...
function T_summary = build_step38_policy_summary_table(T_raw)
...
for i = 1:height(keys)
    ...
    row = table(keys.Scene(i), keys.SceneRole(i), keys.GeometryType(i), ...
        ...
        mean(subT.EmergencyAvgTxRate, 'omitnan'), mean(subT.EmergencyFloorShare, 'omitnan'), ...
...
function T_decision = build_step38_decision_table(T_summary)
keys = unique(T_summary(:, {'Scene','SceneRole','GeometryType','DensityClass','VegetationClass','Config'}), ...
    'rows', 'stable');
methods = ["V4Cost0060", "Risk-constrained-v6"];
rows = [];
...
function T_score = build_step38_score_table(T_decision)
...
function write_step38_report(T_decision, T_score, T_policy_summary, out_path)
fid = fopen(out_path, 'w');
if fid == -1
    error('Could not open Step38 report file: %s', out_path);
end
cleanup_obj = onCleanup(@() fclose(fid)); %#ok<NASGU>

fprintf(fid, '# Step 38 Risk-Constrained V6 Targeted Validation Report\n\n');
fprintf(fid, 'Step38 tests explicit emergency action floors on top of the Step36-selected actual-cost weight (`0.060`).\n\n');
...
fprintf(fid, '\n## Policy Diagnostics\n\n');
fprintf(fid, '| Scene | Config | Method | Direct Share | Redundant Share | Relay Share | Avg Tx Rate | Emergency Floor Share |\n');
fprintf(fid, '|---|---|---|---:|---:|---:|---:|---:|\n');
for i = 1:height(T_policy_summary)
    fprintf(fid, '| %s | %s | %s | %.3f | %.3f | %.3f | %.3f | %.3f |\n', ...
        ...
        T_policy_summary.AvgTxRateMean(i), T_policy_summary.EmergencyFloorShareMean(i));
end
end
