...
% Step34 targeted validation for the performance-anchored risk v3 selector.
% This script preserves Step31/Step32/Step33 outputs and writes Step34 files.
...
prefix = "step34_risk_constrained_v3_targeted_" + string(num_seeds) + "seed";
seed_list = 1:num_seeds;
scene_matrix = get_paper_scene_matrix();
target_cases = build_target_cases();
method_names = ["Confidence-heuristic", "Risk-constrained", ...
    "Risk-constrained-v2", "Risk-constrained-v3"];
...
for case_idx = 1:numel(target_cases)
    ...
    for seed = seed_list
        fprintf('Step34 v3 targeted: scene=%s config=%s seed=%d/%d\n', ...
            target.scene_id, target.config, seed, num_seeds);
        [T_seed, T_stage_seed, T_policy_seed] = simulate_step34_case_seed( ...
...
T_policy_summary = build_step34_policy_summary_table(policy_rows);
T_decision = build_step34_decision_table(T_summary);
T_score = build_step34_score_table(T_decision);
...
write_step34_report(T_decision, T_score, T_policy_summary, prefix + "_report.md");

disp(T_decision);
disp(T_score);
fprintf('Step 34 complete: risk-constrained v3 targeted validation generated with %d seeds.\n', num_seeds);

function num_seeds = get_num_seeds_from_env(default_value)
env_value = str2double(getenv('STEP34_NUM_SEEDS'));
...
error('Unknown Step34 scene id: %s', scene_id);
end

function [T_seed, T_stage_seed, T_policy_seed] = simulate_step34_case_seed( ...
...
results = { ...
    ...
    run_step9_confidence_driven_risk_constrained_v3(scenario, params)};

T_seed = evaluate_step9_metrics(scenario, results);
T_seed = add_step34_meta_columns(T_seed, scene_meta, cfg_name, seed);
...
T_stage_seed = add_step34_meta_columns(T_stage_seed, scene_meta, cfg_name, seed);

T_policy_seed = build_step34_policy_table(scenario, results);
T_policy_seed = add_step34_meta_columns(T_policy_seed, scene_meta, cfg_name, seed);
end

function T = add_step34_meta_columns(T, scene_meta, cfg_name, seed)
...
function T_policy = build_step34_policy_table(scenario, results)
rows = [];
...
for i = 1:numel(results)
    ...
    anchored_replacement_share = NaN;
    emergency_upgrade_share = NaN;
    anchor_share = NaN;
    if isfield(policy, 'selection_reason')
        anchored_replacement_share = mean(policy.selection_reason == "anchored_cost_replacement");
        emergency_upgrade_share = mean(policy.selection_reason == "emergency_safe_upgrade");
        anchor_share = mean(policy.selection_reason == "lagrangian_anchor" ...
            | policy.selection_reason == "relaxed_anchor");
    end

    row = table(string(result.name), mode0, mode1, mode2, avg_priority, avg_tx_rate, ...
        emg_avg_priority, emg_avg_tx_rate, anchored_replacement_share, ...
        emergency_upgrade_share, anchor_share, ...
        'VariableNames', {'Method','DirectShare','RedundantShare','RelayShare', ...
        'AvgPriority','AvgTxRate','EmergencyAvgPriority','EmergencyAvgTxRate', ...
        'AnchoredReplacementShare','EmergencyUpgradeShare','AnchorShare'});
...
function T_summary = build_step34_policy_summary_table(T_raw)
...
for i = 1:height(keys)
    ...
    row = table(keys.Scene(i), keys.SceneRole(i), keys.GeometryType(i), ...
        ...
        mean(subT.EmergencyAvgTxRate, 'omitnan'), mean(subT.AnchoredReplacementShare, 'omitnan'), ...
        mean(subT.EmergencyUpgradeShare, 'omitnan'), mean(subT.AnchorShare, 'omitnan'), ...
        ...
        'AnchoredReplacementShareMean','EmergencyUpgradeShareMean','AnchorShareMean'});
    rows = append_table(rows, row);
end
...
function T_decision = build_step34_decision_table(T_summary)
keys = unique(T_summary(:, {'Scene','SceneRole','GeometryType','DensityClass','VegetationClass','Config'}), ...
    'rows', 'stable');
methods = ["Risk-constrained", "Risk-constrained-v2", "Risk-constrained-v3"];
rows = [];
...
function T_score = build_step34_score_table(T_decision)
...
function write_step34_report(T_decision, T_score, T_policy_summary, out_path)
fid = fopen(out_path, 'w');
if fid == -1
    error('Could not open Step34 report file: %s', out_path);
end
cleanup_obj = onCleanup(@() fclose(fid)); %#ok<NASGU>

fprintf(fid, '# Step 34 Risk-Constrained V3 Targeted Validation Report\n\n');
fprintf(fid, 'Step34 tests a performance-anchored structural selector. ');
fprintf(fid, 'The Step31 Lagrangian action is retained as the anchor, and lower-cost alternatives are accepted only when predicted timely, success, delay, protection, and violation metrics remain close to the anchor. ');
fprintf(fid, 'Emergency messages allow a bounded-cost upgrade when it improves predicted timely delivery.\n\n');
...
fprintf(fid, '\n## Policy Diagnostics\n\n');
fprintf(fid, '| Scene | Config | Method | Direct Share | Redundant Share | Relay Share | Avg Tx Rate | Replacement Share | Emergency Upgrade Share |\n');
fprintf(fid, '|---|---|---|---:|---:|---:|---:|---:|---:|\n');
for i = 1:height(T_policy_summary)
    fprintf(fid, '| %s | %s | %s | %.3f | %.3f | %.3f | %.3f | %.3f | %.3f |\n', ...
        ...
        T_policy_summary.AvgTxRateMean(i), T_policy_summary.AnchoredReplacementShareMean(i), ...
        T_policy_summary.EmergencyUpgradeShareMean(i));
end
