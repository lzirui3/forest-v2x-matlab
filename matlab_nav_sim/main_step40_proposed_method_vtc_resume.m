...
% Step40 resumable VTC validation for the proposed method.
...
num_seeds = get_num_seeds_from_env(50);
prefix = "step40_proposed_method_vtc_" + string(num_seeds) + "seed";
prefix_override = string(getenv('STEP40_PREFIX_TAG'));
...
for scene_idx = 1:numel(scene_matrix)
    ...
    for cfg_idx = 1:numel(config_names)
        ...
        fprintf('Step40 resume: scene=%s config=%s existing=%d missing=%d\n', ...
...
disp(T_decision);
fprintf('Step 40 complete: proposed-method VTC validation generated with %d seeds x %d scenes x %d configs.\n', ...
    num_seeds, numel(scene_matrix), numel(config_names));

function num_seeds = get_num_seeds_from_env(default_value)
env_text = getenv('STEP40_NUM_SEEDS');
if isempty(env_text)
    env_text = getenv('STEP39_NUM_SEEDS');
end
env_value = str2double(env_text);
...
for scene_idx = 1:numel(scene_matrix)
    ...
    for cfg_idx = 1:numel(config_names)
        ...
        if ~isfile(raw_path) || ~isfile(stage_path)
            error('Missing Step40 case output: %s / %s', raw_path, stage_path);
        end
...
if fid == -1
    error('Could not open Step40 report file: %s', out_path);
end
cleanup_obj = onCleanup(@() fclose(fid)); %#ok<NASGU>

fprintf(fid, '# Step 40 Proposed Method VTC Validation Report\n\n');
fprintf(fid, 'Step40 validates the proposed actual-cost-aware risk-constrained emergency-safe scheduler on the full five-scene, two-config matrix. ');
...
fprintf(fid, '\n## Significance Files\n\n');
fprintf(fid, '- Heuristic vs proposed: `step40_*_heuristic_vs_v6_significance.csv`\n');
fprintf(fid, '- Link-delay-aware vs proposed: `step40_*_link_vs_v6_significance.csv`\n');
fprintf(fid, '- V4Cost0060 vs proposed: `step40_*_v4_vs_v6_significance.csv`\n');
