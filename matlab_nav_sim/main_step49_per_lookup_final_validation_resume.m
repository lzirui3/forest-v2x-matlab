...
% Step49: final validation with WiLabV2Xsim PER lookup fixed.
% This is the candidate replacement for Step40 if sigmoid approximation is
% considered too biased by Step47. The script is resumable per scene/config.

base_dir = fileparts(mfilename('fullpath'));
cd(base_dir);

num_seeds = get_env_int_local('STEP49_NUM_SEEDS', 50);
prefix = get_env_string_local('STEP49_PREFIX_TAG', ...
    "step49_per_lookup_final_validation_" + string(num_seeds) + "seed");
scene_ids = get_env_string_list_local('STEP49_SCENES', ...
    ["10014", "10006", "10011", "10019", "10017"]);
config_names = get_env_string_list_local('STEP49_CONFIGS', ["Default", "ForestGeometry"]);
...
for scene_idx = 1:numel(scene_matrix)
    ...
    for cfg = config_names
        ...
        fprintf('Step49 resume: scene=%s config=%s existing=%d missing=%d\n', ...
...
disp(decision);
fprintf('Step49 complete: PER-lookup final validation generated with %d seeds.\n', num_seeds);
...
elseif cfg == "ForestGeometry"
    ...
    error('Unknown Step49 config: %s', cfg);
end
end
...
for scene_idx = 1:numel(scene_matrix)
    for cfg = config_names
        ...
        if ~isfile(raw_path) || ~isfile(stage_path)
            error('Missing Step49 case output: %s / %s', raw_path, stage_path);
        end
...
if timely_h_pp >= 1.0 || emg_h_pp >= 1.0
    label = "PerLookupStrong";
elseif timely_h_pp >= -0.1 && emg_h_pp >= -0.1 && timely_link_pp >= 0.0
    if cost_pct <= 15.0
        label = "PerLookupWeak";
    else
        label = "PerLookupCostly";
    end
else
    label = "PerLookupRisk";
end
...
if fid == -1
    error('Could not open Step49 report file: %s', out_path);
end
cleanup = onCleanup(@() fclose(fid)); %#ok<NASGU>

fprintf(fid, '# Step49 PER-Lookup Final Validation Report\n\n');
fprintf(fid, 'PDR mode is fixed to `per_lookup` for all cases.\n\n');
...
for i = 1:numel(scene_ids)
    ...
    if ~matched
        error('Unknown Step49 scene id: %s', scene_ids(i));
    end
end
end
